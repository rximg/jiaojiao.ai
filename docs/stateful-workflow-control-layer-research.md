...
4. **阶段转换**主要不是由显式状态机驱动，而是由 `SKILL.md` 中的流程约束、`Todo` 状态、工具调用顺序、以及“恢复会话时带入的提示语”共同驱动。换句话说，**业务阶段推进主要是项目自己实现的，而且相当依赖 prompt discipline**。
...
> 当前实现更像“有 durable execution 底座的 Agent 应用”，而不是“业务状态机明确、补偿与重试完备的工作流引擎”。
...
> 阶段转换是当前项目最“业务自研”的一层，而且不是显式状态机实现。
...
## 8.5 路线 E：继续保留 Agent，但在项目内补一个显式状态机层

### 典型做法

- 继续用 `deepagents` 负责 agent/tool
- 但业务上定义 `WorkflowState`
- 每次工具执行前后更新明确状态
- 恢复时根据状态机而不是 prompt 判断下一步
- HITL、retry、compensation 都挂到状态机 transition 上
...
### 结论 C：如果项目继续演进，最值得优先补的不是“换框架”，而是“把控制层显式化”

建议优先级如下：

1. 先把业务状态从 prompt/todo 中抽出来，变成显式 `WorkflowState`
2. 统一 retry policy
3. 为外部副作用引入幂等键和 artifact manifest
4. 把 HITL 语义逐步向 LangGraph 原生 interrupt 靠拢，或明确坚持自定义路线并补齐统一状态机
...
### 方案一：继续留在当前栈，迁移到 LangGraph 显式状态图
...
- 和现有技术栈最兼容
- 不需要推翻 deepagents
- 能把阶段、审批、恢复统一到 graph 语义里

### 方案二：保留现有 agent，但外面包一层业务状态机
